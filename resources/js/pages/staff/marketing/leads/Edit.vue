...
import { index, show, store, update } from '@/routes/staff/marketing/leads';
defineProps<{
...
<template>
    <StaffLayout>
        ...
        <StaffPageHeader title="Edit lead" />
        ...
        <Card>
            ...
            <CardContent>
                <Form
                    v-bind="update.form(lead.id)"
                    class="space-y-6"
                    #default="{ errors, processing }"
                >
                    <div class="grid gap-2">
    <Label for="name">Name</Label>
    <Input id="name" name="name" type="text" :default-value="lead.name" required />
    <InputError :message="errors.name" />
</div>

                <div class="grid gap-2">
    <Label for="email">Email</Label>
    <Input id="email" name="email" type="email" :default-value="lead.email"  />
    <InputError :message="errors.email" />
</div>

                <div class="grid gap-2">
    <Label for="phone">Phone</Label>
    <Input id="phone" name="phone" type="text" :default-value="lead.phone"  />
    <InputError :message="errors.phone" />
</div>

                <div class="grid gap-2">
    <Label for="organisation">Organisation</Label>
    <Input id="organisation" name="organisation" type="text" :default-value="lead.organisation"  />
    ...
    <select id="marketing_advert_id" name="marketing_advert_id" class="border-input h-10 w-full rounded-[var(--radius)] border bg-transparent px-3 text-sm">
        ...
        <option
            ...
            :key="advert.id"
            :value="advert.id"
            :selected="advert.id === lead.marketing_advert_id"
        >
    ...
    <InputError :message="errors.marketing_advert_id" />
                    ...
                    <StaffFormActions :cancel-href="show(lead.id)" :processing="processing" />
                </Form>
            </CardContent>
        </Card>
    </StaffLayout>
